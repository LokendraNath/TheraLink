...
function VapiWidget() {
  ...
  // Wave heights for animation (computed once)
  const [waveHeights] = useState(() =>
    [...Array(5)].map(() => Math.random() * 50 + 20)
  );
    ...
    <div className="max-w-5xl mx-auto px-4 flex flex-col overflow-hidden pb-20">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold font-mono">
          ...
          <span className="text-primary uppercase">AI Mental Assistant</span>
        </h1>
        <p className="text-muted-foreground mt-2">
          Have a voice conversation with our AI assistant for Mental advice and
          guidance
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative">
          <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
            ...
            >
              ...
              <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 flex justify-center items-center h-20">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className={`mx-1 w-1 bg-primary rounded-full ${
                      isSpeaking ? "animate-sound-wave" : ""
                    }`}
                    style={{
                      animationDelay: `${i * 0.1}s`,
                      height: isSpeaking ? `${waveHeights[i]}%` : "5%",
                    }}
                  />
                ))}
              </div>
            ...
            <h2 className="text-xl font-bold text-foreground">TheraLink AI</h2>
            ...
            {messages.map((msg, index) => (
              ...
              >
                <div className="font-semibold text-xs text-muted-foreground mb-1">
                  {msg.role === "assistant" ? "TheraLink AI" : "You"}:
                </div>
            ...
            {callEnded && (
              <div className="message-item animate-in fade-in duration-300">
                ...
                <p className="text-foreground">
                  Call ended. Thank you for using TheraLink AI!
                </p>
              </div>
            )}
          </div>
